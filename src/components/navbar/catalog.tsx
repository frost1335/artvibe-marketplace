...
export default function Catalog({ resorces }: { resorces: any }) {
    return (
        <Sheet>
            <SheetTrigger asChild>
                <Button variant={'secondary'} className="hover:text-indigo-500 hover:bg-indigo-200 dark:hover:bg-indigo-700/50">
                    <div className="flex items-center gap-x-3">
                        ...
                        {resorces.navbar.catalog}
                    </div>
                </Button>
            </SheetTrigger>
            <SheetContent side={'left'}>
                <SheetHeader>
                    <SheetTitle>{resorces.navbar.categories}</SheetTitle>
                </SheetHeader>
                    ...
                    <ul className="space-y-4">
                        {
                            [1, 2, 3, 4, 5, 6, 7].map((category, index) => (
                                <Link className="block" key={index} href={`/uz/category/${category}`}>
